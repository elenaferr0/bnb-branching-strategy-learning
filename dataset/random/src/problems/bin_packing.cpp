...
Problem generate_problem(const int id, const std::pair<int, int> &items, const std::pair<int, int> &bins,
    ...
    const int n_bins = generate_rnd(bins.first, bins.second);
    const double bin_capacity_val = generate_rnd(bin_capacity.first, bin_capacity.second);
    std::vector<double> item_sizes(n_items);
    for (int i = 0; i < n_items; ++i) {
        item_sizes[i] = generate_rnd(item_size.first, item_size.second);
    }
...
std::vector<Problem> bin_packing(const int n_problems,
    ...
    while (generated_problems < n_problems && attempts < n_problems * 2) {
