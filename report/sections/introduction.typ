...
- @MILP problems: some variables, not necessarily all, can take real values.
...
The idea behind this method is to temporarily ignore the integrality constraints on the variables and solve the @LP relaxation of the problem #footnote[The LP relaxation is a modified version of a problem where the integrality constraints on some or all variables are removed, allowing them to take continuous (fractional) values.]\. This is convenient as the relaxation is solvable efficiently using, for instance, the Simplex method @dantzig1951simplex. The @LP relaxation solution provides an optimistical bound for the original @ILP problem; the result might assign a fractional value to one of the originally-integer variables; if this happens, one of them is chosen as a _branching variable_. Let such variable be $x$, then two subproblems are created by adding a new constraint which forces $x <= floor(x)$ and $x >= ceil(x)$ in the left and right subtrees respectively. Since $x$ should have an integer value, it must certainly hold that its value in the solution of the @ILP problem is either less than or equal to its floor or greater than its ceiling. This procedure is then repeated at each node until a solution, not necessarily an optimal one, is found, or the entire search tree has been explored.
...
Ideally, branching rules should produce trees with a limited number of nodes in a reduced execution time. Nevertheless, in practice this is a trade-off. @SB for instance, is arguably the most powerful strategy when it comes to reducing the size, however it is prohibitively expensive to perform, at the point of being unusable in practice for fairly large problems @achterbergWunderling2013MIPSurvey. Conversely, @PCB tries to approximates the accuracy of @SB reducing its computational burden, but it yields bigger trees @bestuzheva2021SCIP.
